...
error-qr-sandbox-failed = 샌드박스에서 QR 코드 디코더를 실행할 수 없습니다
error-qr-sandbox-child-signal = QR 코드 디코더가 시그널 { $signal }에 의해 종료되었습니다
error-qr-sandbox-not-enforced = QR 코드 디코더 샌드박스를 적용할 수 없습니다. 실행 중인 커널이 Landlock을 지원하지 않습니다
